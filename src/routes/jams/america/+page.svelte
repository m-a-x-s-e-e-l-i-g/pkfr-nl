...
<section class="page-hero" style={`--hero-photo: url("${heroPhoto}")`}>
	<div class="hero-content">
		...
		<h1>{$t('jamsAmerica.heading')}</h1>
		<p class="hero-description">{$t('jamsAmerica.intro')}</p>
	</div>
</section>
...
<section class="content-section">
	<div class="section-card">
		<h2>{$t('jamsAmerica.sourceTitle')}</h2>
		<p>{$t('jamsAmerica.sourceDescription')}</p>
		<div class="actions">
			<a href={sourceUrl} rel="noreferrer" target="_blank" class="button">
				{$t('jamsAmerica.sourceButton')}
			</a>
			<a href="/jams" class="button-secondary">{$t('jamsAmerica.backButton')}</a>
		</div>
	</div>
</section>
