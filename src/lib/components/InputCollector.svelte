<script>
    import { useForm, validators, url, pattern } from "svelte-use-form";
...
<div class="formWrapper">
    {#if data.ok}
        <div class="prompt success">
            <p>Bedankt! Je bijdrage zal worden bekeken.</p>
        </div>
    ...
    {:else}
        <form use:form on:submit|preventDefault={sendMessage}>
            <input name="messageInput" use:validators={[url, pattern(youtubeRegex)]} bind:value={messageInput} placeholder="{placeholder}"/>
